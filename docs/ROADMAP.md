...
**What was built**: "Recurring failures" (principles flagged critical/warning across ≥ 3 runs) and "Validated across runs" sections in [patterns-view.tsx](../src/app/components/patterns-view.tsx), ranked by how often real users confirmed them. A per-principle severity timeline (`MiniTimeline`, colored dot per run) and a rising/declining/stable trend badge (`computeTrend`/`TrendIndicator`) are shown on every `AggCard`, plus a stacked bar chart of critical/warning/info/pass counts per run at the top of the page.

**Correction 2026-07-14**: this section previously said the per-principle severity timeline view "was not built." That was stale — `git log` shows `MiniTimeline`/`computeTrend` were added in commit `6edd883` on 2026-06-25, the same day this roadmap was last updated, and just never got reflected here. It's been live for the ~3 weeks since. Nothing left deferred in this item.
